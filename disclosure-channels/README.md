...
1. **`/hand-drawn-canvas-animation create a video animation for this paper's idea of two disclosure channels`**, with the paper's abstract and introduction pasted in. The [`hand-drawn-canvas-animation`](https://github.com/alesha-pro/tools/tree/main/skills/hand-drawn-canvas-animation) skill supplied the engine (`core.js`, `studio.js`, `cels.js`) and its brief-first workflow; Claude wrote a brief (cast, palette, beat sheet, hardest action), a narration script, and generated the voice-over first so that every scene's length would follow the spoken line, not the other way around.
...
Written by Claude Opus 5.5 (`claude-opus-5-5`) in Claude Code, directed by the prompts above, from the abstract and introduction of Brown, Hinson & Tucker (2024, *Contemporary Accounting Research*). Built on the [`hand-drawn-canvas-animation`](https://github.com/alesha-pro/tools/tree/main/skills/hand-drawn-canvas-animation) Claude Code skill, which supplied `core.js`, `studio.js`, `cels.js`, `materials.js` and `render.mjs`. Voice model: [Qwen3-TTS](https://github.com/QwenLM/Qwen3-TTS) by the Qwen team, run locally. Handwriting font: [Caveat](https://fonts.google.com/specimen/Caveat) (SIL OFL).
